FROM        alpine:3.12 AS build-dependencies
...
RUN     buildDeps="autoconf \
                   ...
                   yasm \
                   zlib-dev" && \
        apk  add --update --no-cache ${buildDeps} libgcc libstdc++ ca-certificates libcrypto1.1 libssl1.1
...
#COPY ./libtheora-1.1.1.tar.gz /tmp/theora/
RUN \
        DIR=/tmp/theora && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
...
RUN \
        ...
        tar -zx --strip-components=1 && \
	sed -i '128c #  if defined(PNG_ARM_NEON) && (defined(__ARM_NEON__) || defined(__ARM_NEON)) && \\' thirdparty/libpng/pngpriv.h && \
        cmake -DBUILD_THIRDPARTY:BOOL=ON -DCMAKE_INSTALL_PREFIX="${PREFIX}" . && \
        make && \
        make install && \
        rm -rf ${DIR}
## freetype https://www.freetype.org/
#COPY ./freetype-2.5.5.tar.gz /tmp/freetype/
RUN  \
        DIR=/tmp/freetype && \
        mkdir -p ${DIR} && \
        cd ${DIR} && \
...
#COPY ./master.tar.gz /tmp/aom/master.tar.gz
RUN \
        dir=/tmp/aom ; \
        mkdir -p ${dir} ; \
        cd ${dir} ; \
        ...
        mkdir -p ./aom_build ; \
        cmake -B./aom_build -H. -DCMAKE_INSTALL_PREFIX="${PREFIX}" -DBUILD_SHARED_LIBS=1; \
        cd ./aom_build ; \
        make ; \
        make install ; \
        rm -rf ${dir}
